...
#                "PbP", "MICE.100.Y.IMP", "MICE.100.Y.M.IMP",
#                "MICE.10.Y.M.IMP.M"]  ## MICE RF Methods
...
sel_methods = ["Mean.IMP",  "PbP", "SAEM", 
               "MICE.10.IMP",  "MICE.100.IMP", 
               "MICE.10.Y.IMP", "MICE.100.Y.IMP",
               "MICE.RF.10.M.IMP", "MICE.RF.10.Y.M.IMP",
                # "MICE.10.M.IMP", "MICE.10.Y.M.IMP"
                # "MICE.1000.Y.IMP", "MICE.1000.IMP",
                # "MICE.1000.Y.M.IMP", "MICE.1000.M.IMP"
...
# methods_sel = ["Mean.IMP",  "PbP", "SAEM", "MICE.IMP", "MICE.10.IMP",  "MICE.100.IMP", "MICE.Y.IMP",
#                "MICE.10.Y.IMP", "MICE.100.Y.IMP"]

methods_sel = ["Mean.IMP",  "PbP", "SAEM", 
               "MICE.10.IMP",  "MICE.100.IMP", 
               "MICE.10.Y.IMP", "MICE.100.Y.IMP",
               "MICE.RF.10.M.IMP", "MICE.RF.10.Y.M.IMP",
                # "MICE.10.M.IMP", "MICE.10.Y.M.IMP"
...
plt.tight_layout()
# plt.savefig(os.path.join("plots_scripts", "pwp", "SimD_MAE.pdf"))
# plt.savefig(os.path.join("plots_scripts", "pwp", "SimD_MAE_M.pdf"))
...
methods_sel = ["Mean.IMP",  "PbP", "SAEM", "MICE.IMP", "MICE.10.IMP",  "MICE.100.IMP", "MICE.Y.IMP",
               "MICE.10.Y.IMP", "MICE.100.Y.IMP"]
# methods_sel = ["Mean.IMP",  "PbP", "SAEM", "MICE.10.IMP",  "MICE.100.IMP","MICE.RF.10.IMP",
#                "MICE.10.Y.IMP", "MICE.100.Y.IMP", "MICE.RF.10.Y.IMP" ]  ## MICE RF Methods


scores_sel = ["misclassification", "brier", "calibration", "angular_error", "mse_error"]
filter_bayes = [True, True, True, False, False]

ntrains = [100, 500, 1000, 5000, 10000, 50000]

ylimsmax = [0.12, 0.075, 0.04, 0.2, 1]

ylimsmin = calculate_ymin_for_R_proportion(0.03, ylimsmax)
ylims = [(ylimsmin[i], ylimsmax[i]) for i in range(len(ylimsmax))] 

fig, axes = plt.subplots(1, len(scores_sel), figsize=(4 * len(scores_sel), 6)) # default is 4 * len, 5
if len(scores_sel) == 1:
    axes = [axes]
...
plt.tight_layout()
# plt.savefig(os.path.join("plots_scripts", "pwp", "SimD_other.pdf"))
# plt.savefig(os.path.join("plots_scripts", "pwp", "SimD_other_M.pdf"))
